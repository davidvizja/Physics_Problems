...
**Solution:**
The magnetic field $B$ produced by a long straight wire at distance $r$ is $B = \frac{\mu_0 I}{2 \pi r}$. At the midpoint, the fields from both wires point in the same direction, so we sum them:

$$ B_{\text{total}} = 2 \times \left( \frac{\mu_0 I}{2 \pi r} \right) = \frac{\mu_0 I}{\pi r} $$

**Given:**
*   $I = 5 \text{ A}$
*   $r = 0.05 \text{ m}$ (midpoint of $10 \text{ cm}$)
...
## 3. Biot-Savart Law
**Problem:** A small segment of wire length $0.1 \text{ m}$ carries a current of $3 \text{ A}$ at a distance of $0.2 \text{ m}$ from point $P$.

**Solution:**
Using the Biot-Savart law for a small segment where $\sin(\theta) = 1$:
...
**Solution:**
The torque $\tau$ is maximized when the field is parallel to the loop:

$$ \tau = I \cdot A \cdot B $$

**Given:**
*   $A = 0.10 \text{ m} \times 0.05 \text{ m} = 0.005 \text{ m}^2$
*   $I = 2 \text{ A}$, $B = 0.3 \text{ T}$
...
1.  **Capacitance:**
    $$ C = \frac{\varepsilon_0 S}{d} = 35.4 \text{ pF} $$
2.  **Energy Stored:**
    $$ W = \frac{1}{2} C V^2 = 4.43 \mu\text{J} $$
3.  **Electric Field Intensity:**
    $$ E = \frac{V}{d} = 10^5 \text{ V/m} $$
4.  **Force of Attraction:**
    $$ F = \frac{1}{2} \varepsilon_0 E^2 S = 0.885 \text{ mN} $$
